...
function CardComparer() {
    ...
    function getJackpotFromType(type, sets, setProp) {
        var setProp = setProp || 'set';
        switch (type) {
            case 'trail':
                return compareTrail(sets, setProp);
    ...
    this.getTrailsForJackpot = function(sets, setProp) {
        var arrNew = [],
            sorted,
            setProp = setProp || 'set',
            maxP = -1;
        ...
        sorted = _.sortBy(arrNew, 'priority').reverse();
        trailSets = _.where(sorted, {
            type: 'trail'
        });
        //console.log("trail set:" + JSON.stringify(trailSets));
        if(trailSets.length > 0){
            return trailSets;
        }else{
            return 0;
        }
